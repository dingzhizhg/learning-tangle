...
class Node:
  ...
  def choose_tips(self, num_tips=NUM_TIPS, sample_size=SAMPLE_SIZE, selector=None):
      if selector is None:
    ...
    #   loss_cache = {}
      ...
      for tip in tip_txs:
          topoparam = (LAMBDA*tip.depth + (1-LAMBDA)*tip.brother)
        #   topoparam = 1
          if tip.id in acc_cache.keys():
              score = topoparam*acc_cache[tip.id]
              tip_scores.append((tip, score))
          else:
              self.client.model.set_params(tip.load_weights())
              acc = self.client.test('test')['accuracy']
              score = topoparam*acc
              tip_scores.append((tip, score))
              acc_cache[tip.id] = acc

      best_tips = sorted(tip_scores, key=lambda tup: tup[1], reverse=False)[:num_tips]
  ...
  def process_next_batch(self, num_epochs, batch_size, num_tips=NUM_TIPS, sample_size=SAMPLE_SIZE, reference_avg_top=1):
    # if self.poison_type == PoisonType.NONE:
    selector = TipSelector(self.tangle)
    # else:
